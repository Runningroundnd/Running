...
bot = telebot.TeleBot('7294569410:AAFunicKkISgYtkBqEkAahczfzzcrbr3R04')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"ATTACK BY : {user_name}\n\nTarget : {target}\nPort : {port}\nTime : {time} Seconds\nSend Feedback\n\nᯓ𝐁𝐑𝐀𝐈𝐍 𝐅𝐑𝐄𝐄 𝐃𝐃𝐎𝐒"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 181:
                response = "Wait Until Cooldown Finish ❌.\nPlease Wait 3min Before Running Attack"
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                response = "Error : Time Support Only 180 Sec."
            elif time < 180:
                response = "Error : Time Support Only 180 Sec."
            else:
...
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''👋🏻Welcome to Your Home, {user_name}!\nFeel Free to Explore\n\nWELCOME TO THE BRAIN FREE DDOS SERVER FREEZE BOT'''
    bot.reply_to(message, response)
